...
public class ImportanceToSemiTransparentColorConverter : IValueConverter
{
  ...
  private static bool TryGetColorFromValue(object? value, out Color color)
  {
    switch (value)
    {
      case int intValue and >= 0 and <= 3:
        color = ImportanceColors.SemiTransparentColors[intValue];
        return true;

      case short shortValue and >= 0 and <= 3:
        color = ImportanceColors.SemiTransparentColors[shortValue];
        return true;
