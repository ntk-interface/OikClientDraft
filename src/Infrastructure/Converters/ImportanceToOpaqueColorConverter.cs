...
public class ImportanceToOpaqueColorConverter : IValueConverter
{
  ...
  private static bool TryGetColorFromValue(object? value, out Color color)
  {
    switch (value)
    {
      case int intValue and >= 0 and <= 3:
        color = ImportanceColors.OpaqueColors[intValue];
        return true;

      case short shortValue and >= 0 and <= 3:
        color = ImportanceColors.OpaqueColors[shortValue];
        return true;
